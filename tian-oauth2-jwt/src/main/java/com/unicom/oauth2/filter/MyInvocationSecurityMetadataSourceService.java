...
import com.unicom.oauth2.utils.UrlMatcher;
...
/**
 * @Description: 加载资源与权限的对应关系,实现FilterInvocationSecurityMetadataSource接口也是必须的。 这个类的工作是从数据库中获取授权信息。
 ...
 * 3.容器启动加载顺序：1：调用loadResourceDefine()方法 2：调用supports()方法 3：调用getAllConfigAttributes()方法 @Author:
 * ctf @Date: 2019/5/9
 */
@Component
public class MyInvocationSecurityMetadataSourceService
  ...
  @Autowired private UrlMatcher urlMatcher;

  /**
   * 参数是要访问的url，返回这个url对于的所有权限（或角色） 每次请求后台就会调用 得到请求所拥有的权限 这个方法在url请求时才会调用，服务器启动时不会执行这个方法
  ...
  /** 初始化资源 ,提取系统中的所有权限，加载所有url和权限（或角色）的对应关系， web容器启动就会执行 如果启动@PostConstruct 注解 则web容器启动就会执行 */
  @PostConstruct
  public void loadResourceDefine() {
    ...
    // 获取所有分配的角色
    List<Role> roleList = UserService.ALL_ROLES;
    if (!CollectionUtils.isEmpty(roleList)) {
      for (Role role : roleList) {
        ...
        switch (authorizedSigns) {
          case "ROLE_ADMIN":
            moduleList = UserService.ADMIN_MENUS;
            break;
          case "ROLE_COMMON":
            moduleList = UserService.COMMON_MENUS;
            break;
          default:
        }
